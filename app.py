...
g = geocoder.ip('me')
user_lat = float(g.lat)
user_long = float(g.lng)

def ar_start(img, destin):
    imageTarget = img
    cap = cv2.VideoCapture(0)
    myVid = cv2.VideoCapture('videos/'+ destin + '.mp4')
    detection = False
    frameCounter = 0
    ...
    while True:
        ...
        if len(good) > 12:
            ...
            img2 = cv2.polylines(imgWebcam,[np.int32(dst)],True,(255,0,255),1)
            ...
            imgStacked = stackImages(([imgWebcam, imgVideo, imageTarget],[imgFeatures, imgWarp, imgAug]), 0.5)
...
def findLandmarks(user_lat, user_long):
    # parse
    ...
    for x in range(len(list)):
        ...
        if((b-user_long <=0.05) and (a-user_lat <=0.05)):
            ...
            NearbyLocations.append(list[x].get("Name"))
    travel=input("Enter a location name\n")

    if(travel.isupper() == False):
        travel = travel.upper()

    if travel in NearbyLocations:
        ctr = 1
        while int(ctr) <= 5:
            ctr = str(ctr)
            img = cv2.imread(travel+"/"+ctr+".jpg")
            #write AR code here
            ar_start(img, travel)
...
findLandmarks(user_lat,user_long)